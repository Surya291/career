...
When we started, I told myself this was just an OCR problem. Boy, was I wrong—but usefully so. That belief got me moving and tackling bottlenecks one by one as we dug deeper, and time just flew. It reminds me of Jensen Huang’s comment: if you knew what it really took to build Nvidia, you’d never get started.

I was the technical co-founder and, for most of the company's life, the only full-time engineer. That meant owning the models, the product, and the infra.[^stack] In production we moved roughly 150GB of PDFs, about 1.5 million pages across more than eight hundred Bengaluru projects, serving everyone from 2 BHK buyers to developers moving a thousand flats, villa owners, proptech companies, and SBI, India's largest bank.[^scale] The happy path we sold was real: that multi-day review loop could collapse to a few hours when the machine and a human in the loop both behaved.

We started as a B2B SaaS, billing lawyers monthly (the classic dream), which never quite landed. When LLMs arrived we pivoted toward Outcomes-as-a-Service, stood up an in-house legal team, and the work got meaningfully better. But we could never close the operational gap to reach real startup scale, and we burned out trying. We wound the company down when distribution and the business side of legal tech stopped lining up.[^shutdown]
...
## Making sense of chaos: ingestion and document boundaries
...
We started pre-LLM: OCR text through a NER model, then rule-based stitching to assemble labeled spans into JSON. For a Party Information field in a Sale Deed, NER would tag spans like `[Surya : VNM] [aged 20 : TAG] [care of : TCO] [Mr. Sudarsan : VNM]` and a rule layer would walk the sequence and build the output. My cofounder annotated in Label Studio; I retrained. Roughly forty document types, a mix of structured, semi-structured, and unstructured layouts, and a feedback loop slow enough that adding a new document type felt like a project. For Kannada we trained a multilingual BERT and evaluated with nervaluate[^nervaluate] rather than flat token-level F1, which masks partial-match failures. Still fragile.

When early LLMs became available over API, it was obvious the rule-based downstream was not worth keeping. But context windows were small, so we kept OCR and used NER as a compressor: identify the key text regions, cut those spans, feed the condensed text to the LLM. We also built our own fault-tolerant JSON parser because structured outputs did not exist yet and the models returned near-JSON with regularity.
...
A folder of JSON extracts is not a title opinion. You need a chain of ownership that holds across decades. We modeled the full ownership history as a directed acyclic graph: each node a document, each edge a legal transfer of rights between parties.
...
## When 40 files arrive at 4pm: the distributed backend

Workload shape matters. A lawyer is quiet for two days, then drops a zip of forty files at 4pm and wants progress, not a 504.
...
**Assume inference gets cheaper.** Do not freeze a pipeline around today's price per million tokens. Design so you can widen context, add verification, or swap models when the cost curve moves.

Grateful to the customers who trusted us, to the people who funded the attempt, and to my cofounder. On to the next thing.
